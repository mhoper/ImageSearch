...
class ImageViewZoomExtender implements ZoomableComponent {
	...
	public boolean isScrollOver(Bitmap bitmap, float distanceX){
		if(currentMatrix != null && bitmap != null){
			float m_x = getValue(currentMatrix, Matrix.MTRANS_X);
			float width = imageView.getWidth() - m_x;
			float zoomedWidth = bitmap.getWidth() * getValue(currentMatrix, Matrix.MSCALE_X);
			//放大后：当图片到达最左边界并且继续向右滑 或者到达最右边界继续向左滑 或者图片放大后的宽度小于屏幕宽度
			if((m_x == 0 && distanceX <= 0) || (width == zoomedWidth && distanceX >= 0) || zoomedWidth < imageView.getWidth()){
				return true;
			}
			System.out.println("isScrollOver m_x:" + m_x + " distanceX:" + distanceX + " width:" + width + " bitmap.width:" + bitmap.getWidth() + " scale:" + getValue(currentMatrix, Matrix.MSCALE_X));
		}
